...
import { sendErrorAlert } from "../core/telegram.js";
...
export function registerAdminRoutes(app: Hono): void {
  ...
  const youtubeOAuthStateSchema = z.object({
    channelId: z.string().min(3).max(80),
  ...
  admin.get("/oauth/callback", async (c) => {
    ...
    try {
      ...
      if (config && config.adminToken) {
        const botToken = config.adminToken; // This is actually the telegram bot token in env, but we need the real one
        // We'll send notification via a different approach - using the resolved config's telegram account
      }
