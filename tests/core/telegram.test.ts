...
describe("telegram", () => {
  ...
  it("should handle notifyYoutubeRateLimited with a provided limit", async () => {
    mockSendMessage.mockResolvedValue({ message_id: 112 });
    const { notifyYoutubeRateLimited } = await import("../../src/core/telegram.js");
    await notifyYoutubeRateLimited({ channelName: "Test Channel", reason: "daily-cap", limit: 5 }, { ...mockConfig, telegramBotToken: "token", telegramChatId: "chat_id" } as any);
    expect(mockSendMessage).toHaveBeenCalledTimes(1);
    expect(mockSendMessage.mock.calls[0][1]).toContain("Limite diário de uploads atingido (5).");
  });

  it("should handle notifyYoutubeRateLimited with missing limit", async () => {
    mockSendMessage.mockResolvedValue({ message_id: 111 });
    const { notifyYoutubeRateLimited } = await import("../../src/core/telegram.js");
...
describe("more coverage", () => {
  it("sendToTelegram hits the score and presenter formatting", async () => {
    vi.mocked(fs.statSync).mockReturnValue({ size: 100 } as any);
    vi.mocked(mockSendMessage).mockResolvedValue({ message_id: 111 });
    vi.mocked(mockSendVideo).mockResolvedValue({ message_id: 111 });
    ...
    vi.mocked(mockSendMessage).mockRestore();
    vi.mocked(mockSendVideo).mockRestore();
  });
});
